export default {
  data: {
    zh: '数据',
    en: 'Data',
    vi: 'Data',
  },
  monitoring: {
    zh: '监控',
    en: 'Monitoring',
    vi: 'Giam sat',
  },
  dataMonitoring: {
    zh: '数据监控',
    en: 'Data Monitoring',
    vi: 'Giam sat du lieu',
  },
  history: {
    zh: '历史数据',
    en: 'History',
    vi: 'History',
  },
  current: {
    zh: '当前数据',
    en: 'Current',
    vi: 'Current',
  },
  write: {
    zh: '写数据',
    en: 'Write',
    vi: 'Ghi',
  },
  value: {
    zh: '值',
    en: 'Value',
    vi: 'Gia tri',
  },
  objName: {
    zh: 'Object Name',
    en: 'Object 名称',
    vi: 'Object 名称',
  },
  attribute: {
    zh: '属性',
    en: 'Attribute',
    vi: 'Attribute',
  },
  selectGroupTip: {
    zh: '请先选择 group',
    en: 'Please select group first',
    vi: 'Please select group first',
  },
  updated: {
    zh: '更新时间',
    en: 'Updated',
    vi: 'Cap nhat luc',
  },
  writeDataFormattingErrorPrefix: {
    zh: '类型 ',
    en: 'Type',
    vi: 'Type',
  },
  writeDataFormattingErrorSuffix: {
    zh: ' 内容格式错误',
    en: ' content format error',
    vi: ' content format error',
  },
  arrayLengthError: {
    zh: '请输入长度不大于 {length} 的数据',
    en: 'Please enter data no longer than {length}',
    vi: 'Please enter data no longer than {length}',
  },
  writeDataMinimumErrorPrefix: {
    zh: '小于数据类型 ',
    en: 'Less than the minimum value of data type ',
    vi: 'Less than the minimum value of data type ',
  },
  writeDataMinimumErrorSuffix: {
    zh: ' 的最小值',
    en: '',
    vi: '',
  },
  writeDataMaximumErrorPrefix: {
    zh: '大于数据类型 ',
    en: 'Greater than the maximum value of data type ',
    vi: 'Greater than the maximum value of data type ',
  },
  writeDataMaximumErrorSuffix: {
    zh: ' 的最大值',
    en: '',
    vi: '',
  },
  writeDataSafeMinimumError: {
    zh: `暂不支持输入小于 ${Number.MIN_SAFE_INTEGER} 的数`,
    en: `It is not currently supported to enter a number less than ${Number.MIN_SAFE_INTEGER}`,
    vi: `It is not currently supported to enter a number less than ${Number.MIN_SAFE_INTEGER}`,
  },
  writeDataSafeMaximumError: {
    zh: `暂不支持输入大于 ${Number.MAX_SAFE_INTEGER} 的数`,
    en: `It is not currently supported to enter a number greater than ${Number.MAX_SAFE_INTEGER}.`,
    vi: `It is not currently supported to enter a number greater than ${Number.MAX_SAFE_INTEGER}.`,
  },
  useHexadecimalInput: {
    zh: '使用 16 进制输入',
    en: 'Use hexadecimal input',
    vi: 'Use hexadecimal input',
  },
  hexadecimalError: {
    zh: '请输入格式正确的 16 进制数',
    en: 'Please enter a hexadecimal number in the correct format',
    vi: 'Please enter a hexadecimal number in the correct format',
  },
  displayTheValueInHexadecimal: {
    zh: '以 16 进制展示值',
    en: 'Display the value in hexadecimal',
    vi: 'Display the value in hexadecimal',
  },
  nodeDeleted: {
    zh: '节点不存在',
    en: 'Node not exist',
    vi: 'Node not exist',
  },
  groupDeleted: {
    zh: '组不存在',
    en: 'Group not exist',
    vi: 'Group not exist',
  },
}
